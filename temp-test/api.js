...
exports.toggleSourcesByType = toggleSourcesByType;
exports.toggleSourcesByIds = toggleSourcesByIds;
...
exports.fetchAndProcessNews = fetchAndProcessNews;
exports.getNewsProcessingStatus = getNewsProcessingStatus;
exports.getNews = getNews;
exports.exportToExcel = exportToExcel;
exports.listReports = listReports;
exports.getEmailSettings = getEmailSettings;
exports.updateEmailSettings = updateEmailSettings;
exports.sendNewsEmailSummary = sendNewsEmailSummary;
exports._seedInitialSources = _seedInitialSources;
const db_1 = require("./src/server/db");
const actions_1 = require("./src/server/actions");
const zod_1 = require("zod");
const ExcelJS = __importStar(require("exceljs"));
// Source management
async function listSources() {
    return await db_1.db.source.findMany({
        orderBy: { name: "asc" },
    });
}
async function toggleSource({ id, isEnabled, }) {
    return await db_1.db.source.update({
        where: { id },
        data: { isEnabled },
    });
}
async function toggleSourcesByType({ type, isEnabled, }) {
    return await db_1.db.source.updateMany({
        where: { type },
        data: { isEnabled },
    });
}
async function toggleSourcesByIds({ ids, isEnabled, }) {
    return await db_1.db.source.updateMany({
        where: { id: { in: ids } },
        data: { isEnabled },
    });
...
async function deleteSource({ id }) {
    ...
    const newsItemsCount = await db_1.db.newsItem.count({
        where: { sourceId: id },
    });
    if (newsItemsCount > 0) {
        ...
        await db_1.db.newsItem.updateMany({
            where: { sourceId: id },
            data: { sourceId: null },
        });
    }
    // Delete the source
    return await db_1.db.source.delete({
        where: { id },
    });
}
async function addSource({ name, url, type, }) {
    // Validate source type
    if (!["website", "telegram"].includes(type)) {
        throw new Error("Некорректный тип источника. Допустимые значения: website, telegram");
    }
    // Validate URL format
    ...
    const existingSource = await db_1.db.source.findUnique({
        where: { url },
    });
    if (existingSource) {
        throw new Error("Источник с таким URL уже существует");
    }
    return await db_1.db.source.create({
        data: {
            name,
            url,
            type,
...
async function listKeywords() {
    return await db_1.db.keyword.findMany({
        orderBy: { text: "asc" },
    });
}
async function addKeyword({ text }) {
    return await db_1.db.keyword.create({
        data: { text },
    });
}
async function removeKeyword({ id }) {
    return await db_1.db.keyword.delete({
        where: { id },
    });
}
...
async function fetchAndProcessNews() {
    ...
    const taskId = await (0, actions_1.queueTask)(async () => {
        ...
        for (const source of sources) {
          ...
          СТРОГО ВАЖНО: НЕ ПЕРЕВОДИТЕ КОНТЕНТ НА АНГЛИЙСКИЙ ИЛИ ДРУГИЕ ЯЗЫКИ. Весь текст должен оставаться на оригинальном русском языке. Сохраняйте точное форматирование и оригинальный текст. Обратите особое внимание на дату публикации каждой статьи и на прямую ссылку на конкретную статью.`;
                ...
                const result = await (0, actions_1.requestMultimodalModel)({
                    system: systemPrompt,
                    messages: [
                        {
                            role: "user",
                    ...
                    returnType: zod_1.z.object({
                        articles: zod_1.z.array(zod_1.z.object({
                            title: zod_1.z.string().refine((text) => {
                                ...
                                message: "Заголовок должен быть на русском языке (минимум 90% русских символов, без английских слов)",
                            }),
                            // Строгая проверка на отсутствие английских слов
                            titleNoEnglish: zod_1.z
                                .literal(true)
                                .default(true)
                                .refine(() => {
                            ...
                            summary: zod_1.z.string().refine((text) => {
                                // Очень строгая проверка - текст должен быть почти полностью на русском языке
                                const russianCharsCount = (text.match(/[а-яА-Я]/g) || [])
                                    .length;
                                const totalCharsCount = text.replace(/\s/g, "").length;
                                ...
                                message: "Описание должно быть на русском языке (минимум 90% русских символов, без английских слов)",
                            }),
                            // Строгая проверка на отсутствие английских слов
                            summaryNoEnglish: zod_1.z
                                .literal(true)
                                .default(true)
                                .refine(() => {
                                // Это поле будет проверено позже в коде
                            ...
                            language: zod_1.z.literal("ru").default("ru"), // Явно указываем, что ожидаем ТОЛЬКО русский язык
                        })),
                    }),
                });
                console.log(`Processed source: ${source.name}, found ${result.articles.length} articles`);
                for (const article of result.articles) {
                    ...
                    try {
                        ...
                        const russianDateMatch = article.publishedDate.match(/(\d{1,2})[\.\/-](\d{1,2})[\.\/-](\d{4})/);
                        if (russianDateMatch) {
                            const [_, day, month, year] = russianDateMatch;
                            publishedAt = new Date(Number(year), Number(month) - 1, Number(day));
                        }
                        else {
                            ...
                            for (let i = 0; i < russianMonthNames.length; i++) {
                                ...
                                if (monthVariants) {
                                    for (const monthName of monthVariants) {
                                        ...
                                        const regex = new RegExp(`(\\d{1,2})\\s+${monthName}(?:\\s+(\\d{4}))?`, 'i' // case insensitive
                                        );
                        ...
                        console.error(`Error parsing date: ${article.publishedDate}`, error);
                        publishedAt = new Date(); // Fallback to current date
                    }
                    // Check if article already exists to avoid duplicates
                    const existingArticle = await db_1.db.newsItem.findFirst({
                        where: {
                            title: article.title,
                    ...
                    if (!existingArticle) {
                        ...
                        const russianTitleCharsCount = (article.title.match(/[а-яА-Я]/g) || []).length;
                        const russianSummaryCharsCount = (article.summary.match(/[а-яА-Я]/g) || []).length;
                        const totalTitleCharsCount = article.title.replace(/\s/g, "").length;
                        const totalSummaryCharsCount = article.summary.replace(/\s/g, "").length;
                        // Требуем минимум 80% русских символов
                        const titleIsRussian = russianTitleCharsCount > 0 &&
                            russianTitleCharsCount / totalTitleCharsCount > 0.8;
                        const summaryIsRussian = russianSummaryCharsCount > 0 &&
                        ...
                        if (!titleIsRussian ||
                            ...
                            hasEnglishTitle ||
                            hasEnglishSummary) {
                            console.error(`Пропуск статьи на нерусском языке: ${article.title}`);
                            console.error(`Русских символов в заголовке: ${russianTitleCharsCount}/${totalTitleCharsCount}, в описании: ${russianSummaryCharsCount}/${totalSummaryCharsCount}`);
                            console.error(`Наличие английских слов: в заголовке - ${hasEnglishTitle}, в описании - ${hasEnglishSummary}`);
                            continue;
                        }
                        ...
                        if (englishKeywords.some((keyword) => article.title.includes(keyword) ||
                            article.summary.includes(keyword))) {
                            console.error(`Пропуск статьи с английскими ключевыми словами: ${article.title}`);
                            continue;
                        }
                        ...
                        if (englishSentencePattern.test(article.title) ||
                            englishSentencePattern.test(article.summary)) {
                            console.error(`Пропуск статьи с английскими предложениями: ${article.title}`);
                            continue;
                        }
                        const newsItem = await db_1.db.newsItem.create({
                            data: {
                                title: article.title,
        ...
        // Log the result
        console.log(`Processed ${sources.length} sources, found ${processedNewsItems.length} new articles`);
    });
    return {
        taskId,
...
async function getNewsProcessingStatus({ taskId }) {
    return await (0, actions_1.getTaskStatus)(taskId);
}
// News retrieval
async function getNews({ dateFrom, dateTo, keywords, sourceType, }) {
    // Start building the where condition
    const where = {};
    if (dateFrom) {
        where.publishedAt = Object.assign(Object.assign({}, (where.publishedAt || {})), { gte: new Date(dateFrom) });
    }
    if (dateTo) {
        where.publishedAt = Object.assign(Object.assign({}, (where.publishedAt || {})), { lte: new Date(dateTo) });
    }
    // Add source type filter if provided
    if (sourceType) {
        // First get all sources of the specified type
        const sourcesOfType = await db_1.db.source.findMany({
    ...
    let newsItems = await db_1.db.newsItem.findMany({
        where,
    ...
    if (keywords && keywords.length > 0) {
        newsItems = newsItems.filter((item) => {
            const fullText = `${item.title} ${item.content} ${item.summary} ${item.subject || ""} ${item.position || ""}`.toLowerCase();
...
// Excel export
async function exportToExcel({ dateFrom, dateTo, keywords, }) {
...
async function listReports() {
    return await db_1.db.report.findMany({
...
async function updateEmailSettings({ email, isEnabled, summaryFrequency, }) {
    ...
    return await db_1.db.emailSettings.upsert({
        where: { email },
...
async function checkAndSendEmailSummaries() {
    const emailSettings = await db_1.db.emailSettings.findMany({
        where: { isEnabled: true },
    });
    ...
    for (const settings of emailSettings) {
        try {
            ...
            if (shouldSend) {
                ...
                await db_1.db.emailSettings.update({
...
function shouldSendSummary(settings) {
...
async function sendEmailSummary(email) {
    ...
    const settings = await db_1.db.emailSettings.findUnique({ where: { email } });
    if (!settings)
        return;
    const frequency = settings.summaryFrequency;
    const days = daysToLookBack[frequency] || 1;
    const dateFrom = new Date();
    dateFrom.setDate(dateFrom.getDate() - days);
    const newsItems = await db_1.db.newsItem.findMany({
        where: {
            publishedAt: {
                gte: dateFrom,
            },
    ...
    const websiteNews = newsItems.filter((item) => { var _a; return ((_a = item.source) === null || _a === void 0 ? void 0 : _a.type) === "website"; });
    const telegramNews = newsItems.filter((item) => { var _a; return ((_a = item.source) === null || _a === void 0 ? void 0 : _a.type) === "telegram"; });
    ...
    try {
        await (0, actions_1.sendEmail)(email, `${frequencyText} сводка налоговых новостей`, markdown);
...
async function requestMultimodalModel(params) {
    const response = await (0, actions_1.requestMultimodalModel)(params);
    return response;
}
