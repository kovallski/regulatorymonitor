...
async function sendEmailSummary(email: string) {
  ...
  const settings = await db.emailSettings.findFirst({ where: { email } });
  if (!settings) return;
...
export async function _seedInitialSources() {
  ...
  for (const website of websites) {
    const websiteSource = await db.source.findFirst({ where: { url: website.url } });
    if (!websiteSource) {
  ...
  for (const channel of telegramChannels) {
    const channelSource = await db.source.findFirst({ where: { url: channel.url } });
    if (!channelSource) {
      await db.source.create({
        data: {
          name: channel.name,
